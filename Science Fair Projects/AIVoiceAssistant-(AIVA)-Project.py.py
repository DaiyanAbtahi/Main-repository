...
def input_query():
    ...
    if query.lower() in ["/clr", "/cls", "/clear"]:
        ...
        time.sleep(2)
        result_text.insert(tk.END, f"\n\n{AI_name}: Shayan Afraz and Daiyan Abtahi Razzo\n\n")
        result_text.yview_moveto(1.0)
        root.after(100, lambda: say_text("Shayan Afraz and Daiyan Abtahi Razzo"))
...
def ask_ollama(query):
    ...
    response = ollama.chat(model='llama2-uncensored', messages=[{'role': 'user', 'content': query}])
